    ...
    try {
        ...
        if (existingUser) {
            if (existingUser.m_email === m_email) {
                ...
                return errorHandler.errorThrow(enumConfig.statusErrorCode._422_ERROR[0], '이미 등록된 연락처입니다.');
            }
        }
...
exports.postPasswordResetEmailSend = async (req, res, next) => {
    ...
    try {
        ...
        if (!result) {
            return errorHandler.errorThrow(enumConfig.statusErrorCode._404_ERROR[0], '등록된 이메일 주소가 없습니다.');
        }
...
exports.putPasswordResetEmail = async (req, res, next) => {
    ...
    try {
        ...
        if (minutesDifference > 5) {
            return errorHandler.errorThrow(enumConfig.statusErrorCode._401_ERROR[0], ' 링크 유효기간이 초과했습니다.');
        }
