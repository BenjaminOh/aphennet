...
const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(({ boxClassName, ...props }, ref) => {
    ...
    const textareaStyle = twMerge(
        "w-full h-full resize-none border border-[#D9D9D9] rounded-[5px] focus:border-[#00BA97] p-[12px] text-[14px] placeholder:text-[#999]",
        props.className,
    );
    return (
        <div className={boxStyle}>
            ...
            {props.maxLength && (
                <p className="w-[76px] whitespace-nowrap text-center text-[14px]">
                    <span className="text-[#00BA97]">{props.value?.toString().length ?? 0} </span> / {props.maxLength}
                </p>
            )}
        </div>
    );
