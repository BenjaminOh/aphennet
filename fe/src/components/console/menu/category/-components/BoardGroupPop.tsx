...
export default function BoardGroupPop({ parentId, allUseCheck, handleAllUseCheck }: BoardGroupPopProps) {
    ...
    const {
        ...
        refetch,
    } = useGetBoardGroupList(parentId, { enabled: !!parentId });
    ...
    const handleConfirmStatusChange = (status: "Y" | "N" | "D") => {
        if (checkedList.length === 0) {
            setConfirmPop(true, "분류를 선택해주세요.", 1);
        } else {
            setConfirmPop(true, `${status === "Y" ? "노출" : status === "N" ? "중단" : "삭제"} 하시겠습니까?`, 2, () =>
    ...
    return (
        <ConsoleDialogContent title="분류 설정" className="max-w-[640px]">
            <ScrollArea className="max-h-[75vh] min-h-[300px] border-b border-[#D9D9D9] p-[20px_20px_40px]">
                ...
                <div className="flex items-center justify-between border-b border-[#D9D9D9] p-[20px_0_8px]">
